...
trait FormTrait
{
     ...
     */
    protected function addMaxRule($column, array &$rules): void
    {
        ...
        if (isset($column['length']) && $column['length'] !== '') {
            ...
            foreach ($max_rules as $key => $rule) {
                if ((int) mb_substr($rule, 4) > $column['length']) {
    ...
    private function getSQLLength(string $type)
    {
        ...
        if (mb_strpos($type, '(') !== false) {
            return mb_substr($type, mb_strpos($type, '(') + 1, mb_strpos($type, ')') - mb_strpos($type, '(') - 1);
        }
    ...
    private function getSQLEnumOptions(string $type, bool $nullable = false)
    {
        ...
        $values = explode(',', str_replace("'", '', mb_substr($type, mb_strpos($type, '(') + 1, mb_strpos($type, ')') - mb_strpos($type, '(') - 1)));
